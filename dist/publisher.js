...
async function releaseLockAndScheduleRetry(eventId, error) {
...
async function markPublishedAndUnlock(eventId) {
    await prisma.event.update({
        where: { eventId },
        data: {
...
async function tick() {
    ...
    try {
        ...
        if (batch.length === 0)
            return;
        fastify.log.info({ count: batch.length, lockId }, 'Claimed outbox events');
