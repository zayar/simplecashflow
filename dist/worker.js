...
async function handleJournalEntryCreated(event) {
    ...
    if (!eventId || !companyId || !journalEntryId) {
        fastify.log.error({ event }, 'Missing eventId, companyId, or journalEntryId in event payload');
        return;
    }
    try {
        ...
        if (err.code === 'P2002') {
            fastify.log.warn({ eventId }, 'Duplicate event detected, skipping (idempotent)');
            return;
        }
