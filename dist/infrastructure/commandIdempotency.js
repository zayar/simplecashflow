 ...
 */
export async function runIdempotentRequest(prisma, companyId, key, work, redis, redisTtlMs = 86_400_000 // 24h
) {
    ...
    if (redis) {
        try {
            ...
            if (cached) {
                try {
                    return { replay: true, response: JSON.parse(cached) };
                }
                catch {
                    // fall through to DB
                }
    ...
    try {
        ...
        if (err?.code === 'P2002') {
            ...
            if (nowExisting)
                return { replay: true, response: nowExisting.response };
        }
...
//# sourceMappingURL=commandIdempotency.js.map