...
export function _test_replayStockMovesWithBackdatedValueInsert(args) {
    ...
    for (const m of moves) {
        ...
        if (m.type === 'VALUE_ADJUSTMENT' && m.direction === 'IN' && d2(m.quantity).equals(0)) {
...
export async function applyStockValueAdjustmentWac(tx, input) {
    ...
    if (newV.lessThan(0))
        throw Object.assign(new Error('inventory value cannot be negative'), { statusCode: 400 });
    const newA = d2(newV.div(Q));
    const balance = await tx.stockBalance.upsert({
        where: {
            companyId_locationId_itemId: {
    ...
    return { balance, move, requiresInventoryRecalcFromDate: null };
}
 ...
 */
export async function ensureInventoryCompanyDefaults(tx, companyId) {
    const company = await tx.company.findUnique({
        where: { id: companyId },
        select: {
            id: true,
    ...
    // 2) Accounts
    let inventoryAssetAccountId = company.inventoryAssetAccountId ?? null;
    let cogsAccountId = company.cogsAccountId ?? null;
    let openingBalanceEquityAccountId = company.openingBalanceEquityAccountId ?? null;
...
export async function ensureInventoryItem(tx, companyId, itemId) {
    ...
    if (item.type !== 'GOODS')
        throw Object.assign(new Error('only GOODS can be inventoried'), { statusCode: 400 });
 ...
 */
export async function applyStockMoveWac(tx, input) {
