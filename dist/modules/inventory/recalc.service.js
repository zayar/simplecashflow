...
export function _test_applyWacReplay(args) {
    ...
    const updatedOutMoves = [];
    const deltaByJournalEntryId = new Map();
...
async function computeBaselineByKey(tx, companyId, startInclusive) {
...
async function loadMovesFrom(tx, companyId, fromInclusive, toInclusive) {
...
async function upsertStockBalancesFromState(tx, companyId, ending) {
...
async function applyOutMoveRevaluations(tx, companyId, updates) {
...
async function adjustCogsForJournalEntries(tx, companyId, deltas) {
    if (deltas.size === 0)
    ...
    `);
        ...
        if (delta.equals(0))
            continue;
        const lines = delta.greaterThan(0)
            ? [
...
export async function runInventoryRecalcForward(tx, args) {
...
export async function rebuildProjectionsFromLedger(tx, args) {
    const companyId = args.companyId;
    const fromDate = normalizeToDay(args.fromDate);
    const toDate = normalizeToDay(args.toDate);
    // Clear existing projections in range
    await tx.accountBalance.deleteMany({ where: { companyId, date: { gte: fromDate, lte: toDate } } });
    await tx.dailySummary.deleteMany({ where: { companyId, date: { gte: fromDate, lte: toDate } } });
  ...
  `);
    ...
    for (let i = 0; i < abData.length; i += chunkSize) {
        ...
        await tx.accountBalance.createMany({ data: chunk, skipDuplicates: true });
    }
  ...
  `);
    ...
    for (let i = 0; i < dsData.length; i += chunkSize) {
        ...
        await tx.dailySummary.createMany({ data: chunk, skipDuplicates: true });
    }
  ...
  `);
    if (eventIds.length > 0) {
        ...
        for (let i = 0; i < peData.length; i += 1000) {
            ...
            await tx.processedEvent.createMany({ data: chunk, skipDuplicates: true });
        }
    }
    return { companyId, fromDate: fromDate.toISOString(), toDate: toDate.toISOString(), rebuilt: true };
}
//# sourceMappingURL=recalc.service.js.map