...
async function ensureCashAccount1000(tx, companyId) {
    const existing = await tx.account.findFirst({
...
export async function upsertPostedInvoiceFromPitiSale(args) {
    ...
    if (!payload.saleId || !String(payload.saleId).trim())
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (existingMap?.internalId) {
            ...
            if (!inv)
                throw new Error('integration mapping exists but invoice not found');
        ...
        if (!company.accountsReceivableAccountId)
            throw new Error('company.accountsReceivableAccountId is not set');
        ...
        if (!arAccount)
            throw new Error('accountsReceivableAccountId must be an ASSET account in this company');
        ...
        if (customerExtId) {
            ...
            if (cm?.internalId)
                customerId = Number(cm.internalId);
        }
        ...
        if (!customerId) {
            if (!autoCreateCustomer)
                throw new Error('customer not mapped and autoCreateCustomer=false');
        ...
        for (const l of payload.lines) {
            if (!l || !l.quantity || l.quantity <= 0)
                throw new Error('each line must have quantity > 0');
            if (!l.unitPrice || l.unitPrice <= 0)
                throw new Error('each line must have unitPrice > 0');
            ...
            if (!item) {
                if (!autoCreateItems)
                    throw new Error(`item not mapped for externalProductId=${extProductId ?? 'null'} sku=${sku ?? 'null'}`);
            ...
            if (discount.lessThan(0) || discount.greaterThan(lineSubtotal))
                throw new Error('discountAmount must be between 0 and line subtotal');
        ...
        // Invoice totals
        const linesForMath = computedLines.map((l) => ({
            quantity: l.quantity,
        ...
        const d = jeLines.reduce((s, l) => s.add(l.debit), new Prisma.Decimal(0)).toDecimalPlaces(2);
        const c = jeLines.reduce((s, l) => s.add(l.credit), new Prisma.Decimal(0)).toDecimalPlaces(2);
        if (!d.equals(c))
            throw new Error('journal entry imbalance');
        ...
        assertTotalsMatchStored(total, new Prisma.Decimal(inv.total));
        ...
        if (recordPayment && payments.length > 0) {
            for (const p of payments) {
                ...
                if (!bankAccount)
                    throw new Error('payment bank/cash account must be an ASSET account in this company');
...
export async function upsertPostedCreditNoteFromPitiRefund(args) {
    const { prisma, companyId, payload } = args;
    if (!payload.refundId || !String(payload.refundId).trim())
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        if (existingMap?.internalId) {
            ...
            if (!cn)
                throw new Error('integration mapping exists but credit note not found');
            return {
        ...
        if (payload.saleId) {
            ...
            if (saleMap?.internalId)
                invoiceId = Number(saleMap.internalId);
        }
        ...
        await tx.creditNote.update({ where: { id: cn.id }, data: { journalEntryId: je.id } });
        // Events
        const jeEventId = randomUUID();
        await tx.event.create({
            data: {
...
//# sourceMappingURL=piti.service.js.map