...
function requireInt(name, v, min, max) {
    const n = Number(v);
...
export async function cashflowRoutes(fastify) {
    ...
    fastify.put('/companies/:companyId/cashflow/settings', async (request, reply) => {
        ...
        const data = {};
        if (body.defaultArDelayDays !== undefined)
            data.defaultArDelayDays = requireInt('defaultArDelayDays', body.defaultArDelayDays, 0, 180);
        if (body.defaultApDelayDays !== undefined)
            data.defaultApDelayDays = requireInt('defaultApDelayDays', body.defaultApDelayDays, 0, 180);
        if (body.minCashBuffer !== undefined)
            data.minCashBuffer = requireMoney('minCashBuffer', body.minCashBuffer);
        ...
        return await prisma.cashflowSettings.upsert({
            where: { companyId },
            create: { companyId, defaultArDelayDays: 7, defaultApDelayDays: 0, minCashBuffer: 0, ...data },
            update: data,
        });
    });
    // ---------------------------------------------------------------------------
    ...
    fastify.get('/companies/:companyId/cashflow/forecast', async (request, reply) => {
        ...
        const scenario = (String(q?.scenario ?? 'base').trim().toLowerCase() || 'base');
        ...
        if (snap?.payload && typeof snap.payload === 'object') {
            return { ...snap.payload, computedAt: snap.computedAt.toISOString(), source: 'snapshot' };
        }
        const opts = { weeks, scenario, asOfDate: day };
        const result = await prisma.$transaction(async (tx) => {
            return await computeCashflowForecast(tx, companyId, opts);
        });
        ...
        return { ...result, computedAt: new Date().toISOString(), source: 'computed' };
    });
    fastify.get('/companies/:companyId/cashflow/alerts', async (request, reply) => {
        ...
        const result = await prisma.$transaction(async (tx) => {
            return await computeCashflowForecast(tx, companyId, { weeks: 13, scenario });
        });
    ...
    fastify.get('/companies/:companyId/cashflow/insights', async (request, reply) => {
        ...
        const userId = Number(request?.user?.userId ?? 0) || null;
    ...
    fastify.put('/companies/:companyId/cashflow/recurring-items/:id', async (request, reply) => {
        ...
        if (body.currency !== undefined)
            data.currency = normCurrency(body.currency);
        ...
        if (body.interval !== undefined)
            data.interval = requireInt('interval', body.interval, 1, 52);
