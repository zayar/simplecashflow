...
function normalizeCurrencyOrThrow(input, fieldName = 'currency') {
...
function parseIsoDateOrThrow(input, fieldName = 'asOfDate') {
...
export async function currenciesRoutes(fastify) {
    ...
    fastify.put('/companies/:companyId/currencies/:currencyId', async (request, reply) => {
        ...
        const body = request.body;
        const existing = await prisma.currency.findFirst({ where: { id: currencyId, companyId } });
        if (!existing) {
            reply.status(404);
    ...
    fastify.get('/companies/:companyId/currencies/:code/rates', async (request, reply) => {
        const companyId = requireCompanyIdParam(request, reply);
        const code = normalizeCurrencyOrThrow(request.params?.code, 'code');
        const company = await prisma.company.findUnique({ where: { id: companyId }, select: { baseCurrency: true } });
        const baseCurrency = (company?.baseCurrency ?? '').trim().toUpperCase() || null;
        if (!baseCurrency) {
            reply.status(400);
        ...
        const currency = await prisma.currency.findFirst({ where: { companyId, code, isActive: true } });
        if (!currency) {
            reply.status(404);
            return { error: 'currency not found' };
        }
