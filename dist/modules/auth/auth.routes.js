...
import { AccountType, BankingAccountKind, CashflowActivity, NormalBalance, UserRole, } from '@prisma/client';
export async function authRoutes(fastify) {
    ...
    const OTP_DEBUG = (process.env.OTP_DEBUG ?? '').toLowerCase() === 'true' ||
    ...
    async function bestEffortOtpGuards(phone) {
    ...
    function validatePasswordPolicy(password) {
        ...
        if (!hasLower || !hasUpper || !hasDigit) {
            throw Object.assign(new Error('password must include at least 1 uppercase letter, 1 lowercase letter, and 1 number'), { statusCode: 400 });
        }
    }
    ...
    fastify.post('/login', async (request, reply) => {
        ...
        return {
            token,
            user: { id: user.id, email: user.email, name: user.name, companyId: user.companyId, role: user.role ?? UserRole.OWNER },
        };
    });
    ...
    fastify.post('/me/phone/request-otp', { preHandler: fastify.authenticate }, async (request, reply) => {
        const body = request.body;
        const jwtUser = request.user;
        const userId = Number(jwtUser?.userId);
        const companyId = Number(jwtUser?.companyId);
    ...
    fastify.post('/me/phone/verify', { preHandler: fastify.authenticate }, async (request, reply) => {
        const body = request.body;
        const jwtUser = request.user;
        const userId = Number(jwtUser?.userId);
        const companyId = Number(jwtUser?.companyId);
