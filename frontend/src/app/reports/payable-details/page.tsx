...
export default function PayableDetailsReportPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await fetchApi(`/companies/${user.companyId}/reports/payable-details?${qs.toString()}`);
      setData(res);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <h1 className="text-2xl font-semibold tracking-tight">Payable Details</h1>
        <p className="text-sm text-muted-foreground">Bill line-level payables for a date range.</p>
      </div>
      ...
      <Card className="shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <CardTitle className="text-lg">Payable Details</CardTitle>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Item Price (BCY)</TableHead>
                  <TableHead className="text-right">Total (BCY)</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {(data?.rows ?? []).map((r: any, idx: number) => (
                  <TableRow key={`${r.transactionNumber}-${idx}`}>
                    ...
                    <TableCell className="text-right tabular-nums">{Number(r.quantityOrdered ?? 0).toLocaleString()}</TableCell>
                    <TableCell className="text-right tabular-nums">
                      {Number(r.itemPriceBCY ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </TableCell>
                ...
                {(data?.rows ?? []).length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={10} className="py-10 text-center text-muted-foreground">
                      No payable details found for this period.
                    </TableCell>
                  </TableRow>
                ) : null}
              </TableBody>
