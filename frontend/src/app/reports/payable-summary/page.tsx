...
export default function PayableSummaryReportPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await fetchApi(`/companies/${user.companyId}/reports/payable-summary?${qs.toString()}`);
      setData(res);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <h1 className="text-2xl font-semibold tracking-tight">Payable Summary</h1>
        <p className="text-sm text-muted-foreground">Bill-level payables for a date range.</p>
      </div>
      ...
      <Card className="shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <CardTitle className="text-lg">Payable Summary</CardTitle>
