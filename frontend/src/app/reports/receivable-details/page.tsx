...
export default function ReceivableDetailsReportPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await fetchApi(`/companies/${user.companyId}/reports/receivable-details?${qs.toString()}`);
      setData(res);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <h1 className="text-2xl font-semibold tracking-tight">Receivable Details</h1>
        <p className="text-sm text-muted-foreground">Invoice line-level receivables for a date range.</p>
      </div>
                ...
                {(data?.rows ?? []).length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={10} className="py-10 text-center text-muted-foreground">
                      No receivable details found for this period.
                    </TableCell>
                  </TableRow>
                ) : null}
              </TableBody>
