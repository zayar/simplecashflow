...
import { createVendor } from '../lib/expenses';
...
export default function VendorNew() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const created = await createVendor(companyId, {
        name: name.trim(),
      ...
      if (mode === 'pick') {
        // Store in sessionStorage for the expense form to pick up
        try {
          sessionStorage.setItem('picked_vendor', JSON.stringify({ id: created.id, name: created.name }));
        } catch {}
        navigate(-1);
      } else {
