...
export async function taxesRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/companies/:companyId/tax-rates/:taxRateId', async (request, reply) => {
    const companyId = requireCompanyIdParam(request, reply);
    const taxRateId = Number((request.params as any)?.taxRateId);
  ...
  fastify.post('/companies/:companyId/tax-rates', async (request, reply) => {
    ...
    const body = request.body as {
      name?: string;
      rate?: number; // can be percentage (10) or decimal (0.10)
      isCompound?: boolean;
    };
  ...
  fastify.put('/companies/:companyId/tax-rates/:taxRateId', async (request, reply) => {
    ...
    if (body.isCompound !== undefined) updateData.isCompound = body.isCompound;
  ...
  fastify.get('/companies/:companyId/tax-groups/:taxGroupId', async (request, reply) => {
    const companyId = requireCompanyIdParam(request, reply);
    const taxGroupId = Number((request.params as any)?.taxGroupId);
  ...
  fastify.post('/companies/:companyId/tax-groups', async (request, reply) => {
    ...
    const body = request.body as {
      name?: string;
      taxRateIds?: number[]; // array of tax rate IDs to include
    };
