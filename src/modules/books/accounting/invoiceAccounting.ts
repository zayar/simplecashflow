...
export type InvoiceLineInput = {
  ...
  taxRate?: Decimal | number | string | null; // 0..1
  incomeAccountId: number;
};

export type JournalLine = { accountId: number; debit: Decimal; credit: Decimal };

function d2(n: Decimal): Decimal {
  return n.toDecimalPlaces(2);
}

function d4(n: Decimal): Decimal {
...
export function computeInvoiceTotalsAndIncomeBuckets(lines: InvoiceLineInput[]) {
  let subtotal = new Decimal(0);
  let taxAmount = new Decimal(0);
  const incomeBuckets = new Map<number, Decimal>();
...
export function assertTotalsMatchStored(total: Decimal, storedTotal: Decimal) {
  const t = d2(total);
...
export function buildInvoicePostingJournalLines(args: {
  ...
  if (taxAmount.greaterThan(0)) {
    if (!args.taxPayableAccountId) throw new Error('taxPayableAccountId required when taxAmount > 0');
  ...
  if (totalCogs.greaterThan(0)) {
    ...
    lines.push(
      { accountId: args.cogsAccountId, debit: totalCogs, credit: new Decimal(0) },
      { accountId: args.inventoryAssetAccountId, debit: new Decimal(0), credit: totalCogs }
    );
  }
...
export function sumDebitsCredits(lines: JournalLine[]) {
