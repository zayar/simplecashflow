...
export async function customerAdvancesRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/companies/:companyId/customer-advances', async (request, reply) => {
    ...
    try {
      const { response: result } = await withLockBestEffort(redis, lockKey, 30_000, async () =>
        runIdempotentRequest(prisma, companyId, idempotencyKey, async () => {
          const txResult = await prisma.$transaction(async (tx: any) => {
            ...
            if (!liabAcc) throw Object.assign(new Error('liabilityAccountId not found in this company'), { statusCode: 400 });
      ...
      return {
        customerAdvanceId: (result as any).customerAdvanceId,
  ...
  fastify.post('/companies/:companyId/invoices/:invoiceId/apply-credits', async (request, reply) => {
    ...
    const body = (request.body ?? {}) as { customerAdvanceId?: number; amount?: number; appliedDate?: string };
    ...
    try {
      const { response: result } = await withLockBestEffort(redis, lockKey, 30_000, async () =>
        runIdempotentRequest(prisma, companyId, idempotencyKey, async () => {
          const txResult = await prisma.$transaction(async (tx: any) => {
            ...
            if (!arId) throw Object.assign(new Error('Company AR account is not configured'), { statusCode: 400 });
            ...
            if (!adv) throw Object.assign(new Error('customer advance not found'), { statusCode: 404 });
      ...
      return {
        ...
        customerAdvanceApplicationId: (result as any).customerAdvanceApplicationId,
        journalEntryId: (result as any).journalEntryId,
  ...
  fastify.post('/companies/:companyId/credit-notes/:creditNoteId/unapply', async (request, reply) => {
    ...
    try {
      const { response: result } = await withLockBestEffort(redis, lockKey, 30_000, async () =>
        runIdempotentRequest(prisma, companyId, idempotencyKey, async () => {
          const txResult = await prisma.$transaction(async (tx: any) => {
            ...
            const newStatus =
              settled.greaterThanOrEqualTo(new Prisma.Decimal(inv.total).toDecimalPlaces(2))
                ? 'PAID'
            ...
            await tx.invoice.updateMany({
              where: { id: inv.id, companyId },
              data: { amountPaid: settled, status: newStatus } as any,
            });
      ...
      return { creditNoteId: (result as any).creditNoteId, invoiceId: (result as any).invoiceId, status: (result as any).status };
    } catch (err: any) {
      if (err?.statusCode) {
        reply.status(err.statusCode);
        return { error: err.message };
