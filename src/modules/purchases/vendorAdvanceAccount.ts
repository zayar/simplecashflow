 ...
 */
export async function ensureVendorAdvanceAccount(tx: any, companyId: number): Promise<number> {
  ...
  const byName = new Map<string, { id: number; name: string; code: string }>();
  ...
  if (legacyIds.length > 0) {
    ...
    for (const g of grouped ?? []) {
      ...
      if (debit.equals(0) && credit.equals(0)) continue;
      await tx.accountBalance.upsert({
        ...
        update: {
          debitTotal: { increment: debit } as any,
          creditTotal: { increment: credit } as any,
        },
        create: {
          companyId,
          accountId: canonicalId,
