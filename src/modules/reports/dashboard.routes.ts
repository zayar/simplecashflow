...
export async function dashboardRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/companies/:companyId/dashboard', async (request, reply) => {
    ...
    async function accountBalanceAsOf(accountId: number | null): Promise<Prisma.Decimal> {
    ...
    const cashAccounts = await prisma.account.findMany({
      where: { companyId, type: 'ASSET', reportGroup: 'CASH_AND_CASH_EQUIVALENTS' as any },
      select: { id: true },
    });
    ...
    async function balanceAsOfForAccounts(accountIds: number[], asOf: Date, normalBalance: 'DEBIT' | 'CREDIT') {
      if (accountIds.length === 0) return new Prisma.Decimal(0);
      const sums = await prisma.accountBalance.aggregate({
    ...
    const movements = movementAgg
      ...
      .filter(Boolean) as Array<{ accountId: number; code: string; name: string; movement: string; abs: Prisma.Decimal }>;

    movements.sort((a, b) => b.abs.comparedTo(a.abs));

    // --- Trend: Income vs Expense (last 12 months ending at toDate) ---
    const trendMonths: Array<{ label: string; from: string; to: string; income: string; expense: string }> = [];
    ...
    const expRows = expenseAgg
      ...
      .filter(Boolean) as Array<{ accountId: number; code: string | null; name: string | null; amount: Prisma.Decimal }>;
