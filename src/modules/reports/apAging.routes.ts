...
function bucketForDaysPastDue(d: number): 'CURRENT' | 'DUE_1_30' | 'DUE_31_60' | 'DUE_61_90' | 'DUE_90_PLUS' {
  if (d <= 0) return 'CURRENT';
  if (d <= 30) return 'DUE_1_30';
  if (d <= 60) return 'DUE_31_60';
  if (d <= 90) return 'DUE_61_90';
  return 'DUE_90_PLUS';
}

export async function apAgingRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/companies/:companyId/reports/ap-aging', async (request, reply) => {
    ...
    const bills = await prisma.expense.findMany({
      where: { companyId, status: { in: ['POSTED', 'PARTIAL'] } as any },
      include: { vendor: true },
    });
    ...
    type Row = {
      ...
      due_1_30: Prisma.Decimal;
      due_31_60: Prisma.Decimal;
      due_61_90: Prisma.Decimal;
      due_90_plus: Prisma.Decimal;
      total: Prisma.Decimal;
    };

    const byVendor = new Map<string, Row>(); // key `${vendorId ?? 'null'}`
    const ensure = (vendorId: number | null, vendorName: string): Row => {
    ...
    const apply = (vendorId: number | null, vendorName: string, dueDate: Date | null, amount: Prisma.Decimal) => {
      ...
      if (bucket === 'CURRENT') row.current = row.current.add(amount);
      else if (bucket === 'DUE_1_30') row.due_1_30 = row.due_1_30.add(amount);
      else if (bucket === 'DUE_31_60') row.due_31_60 = row.due_31_60.add(amount);
      else if (bucket === 'DUE_61_90') row.due_61_90 = row.due_61_90.add(amount);
      else row.due_90_plus = row.due_90_plus.add(amount);
      row.total = row.total.add(amount);
    };

    for (const b of bills as any[]) {
      const outstanding = new Prisma.Decimal(b.amount).minus(new Prisma.Decimal((b as any).amountPaid ?? 0)).toDecimalPlaces(2);
      if (outstanding.lessThanOrEqualTo(0)) continue;
      apply(b.vendorId ?? null, b.vendor?.name ?? 'No Vendor', (b as any).dueDate ?? null, outstanding);
    }

    for (const pb of purchaseBills as any[]) {
      const outstanding = new Prisma.Decimal(pb.total).minus(new Prisma.Decimal((pb as any).amountPaid ?? 0)).toDecimalPlaces(2);
      if (outstanding.lessThanOrEqualTo(0)) continue;
