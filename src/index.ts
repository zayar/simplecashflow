...
await fastify.register(cors, {
  origin: true, // Allow all origins for dev (or set to 'http://localhost:3000')
});
...
function parseCompanyId(params: any): number | null {
...
async function publishDomainEvent(event: DomainEventEnvelopeV1): Promise<boolean> {
  try {
...
async function markEventPublished(eventId: string) {
  try {
