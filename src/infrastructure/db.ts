...
rawPrisma.$use(async (params, next) => {
  ...
  if (!isPerfEnabled() || !getPerfStore()) return next(params);
  ...
  try {
    ...
    const sample: { ms: number; model?: string; operation?: string } = { ms };
    if (params.model) sample.model = String(params.model);
    if (params.action) sample.operation = String(params.action);
